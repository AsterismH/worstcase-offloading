    ...
    def serveByTaskNum(self, num, timer):
        ...
        while len(self.taskArray) > 0 and num > 0:
            ...
            if len(self.taskArray) > 0:
                ...
                self.currentTask = None
        return workload
    def serveByTaskList(self, serveCycle, taskList, timer):
        self.checkForLmax(timer)
        if taskList == []:
            return
        ...
        assert self.currentTask != None
        assert self.currentTask.remainCycle > 0
        assert serveCycle > self.currentTask.remainCycle + workloadOfSet(taskList[:-1]) \
            and serveCycle <= self.currentTask.remainCycle + workloadOfSet(taskList)
        ...
        serveCycle -= self.currentTask.remainCycle
        self.length -= self.currentTask.remainCycle
        self._rountineForServedTask(self.currentTask, timer)
        # serve out all tasks in taskList except the last one
        serveCycle -= workloadOfSet(taskList[:-1])
        self.length -= workloadOfSet(taskList[:-1])
        for i in taskList[:-1]:
            self._rountineForServedTask(i, timer)
        # serve the last one
        assert serveCycle > 0 and serveCycle <= taskList[-1].totalCycle
        if serveCycle == taskList[-1].totalCycle:
            self.length -= taskList[-1].remainCycle
            self._rountineForServedTask(taskList[-1], timer)
            # pick next HOL task
            if len(self.taskArray) > 0:
                self.currentTask = self.taskArray[0]
            else:
            ...
            i = taskList[-1]
            self.taskArray.insert(0, i)
            self.currentTask = i
            i.currentQueue = self
            i.remainCycle -= serveCycle
            self.length -= serveCycle
            i.status = 'SERVING'
    ...
    def offloadByCycle(self, cycle):
        ...
        while len(self.taskArray) > 1:
            if cycle > self.taskArray[-1].remainCycle:
                ...
                break
        return taskList
    def pickTask(self): # TODO: should delete
        if len(self.taskArray) == 0:
            return None
        else:
            return self.taskArray[0]
